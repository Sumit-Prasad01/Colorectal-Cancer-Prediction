...
class DataProcessing:
    ...
    def preprocess_data(self):
        try:
            ...
            logger.info("Data basic processing done")

        except Exception as e:
            logger.error(f"Error while preprocessing data {e}")
            raise CustomException("Failed to perprocess data")
    ...
    def feature_selection(self, ):
        try:
            ...
            logger.info(f"Feature Importance : {chi2_scores}")
            ...
            logger.info("Featrure selection done.")
        
        except Exception as e:
            logger.error(f"Error while feature selection {e}")
            raise CustomException("Failed to select features.")
    ...
    def run(self):
        try:
            ...
            logger.error(f"Error while running data processing pipeline {e}")
            raise CustomException("Failed to run dat aprocessing pipeline.")
